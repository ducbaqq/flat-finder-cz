    ...
    for city_name, city_info in cities_data.items():
        for i in range(city_info["count"]):
            ...
            listings.append((
                ...
                thumbnail_url, source_urls[source], listed_at
            ))
...
def api_stats():
    ...
    by_source = {}
    for row in db.execute("SELECT source, COUNT(*) as cnt FROM listings GROUP BY source"):
        by_source[row["source"]] = row["cnt"]

    by_type = {}
    for row in db.execute("SELECT property_type, COUNT(*) as cnt FROM listings GROUP BY property_type"):
        by_type[row["property_type"]] = row["cnt"]

    by_transaction = {}
    for row in db.execute("SELECT transaction_type, COUNT(*) as cnt FROM listings GROUP BY transaction_type"):
        by_transaction[row["transaction_type"]] = row["cnt"]

    by_city = {}
    for row in db.execute("SELECT city, COUNT(*) as cnt FROM listings GROUP BY city ORDER BY cnt DESC LIMIT 10"):
