    ...
    for listing in listings:
        try:
            ...
            if db.execute("SELECT changes()").fetchone()[0] > 0:
                ...
                # Listing already exists — reactivate if it was inactive
                reactivate_listing(db, listing["external_id"])
...
def deactivate_stale_listings(db, source, current_external_ids):
    """
    Mark listings as inactive if they were not seen in the latest collection.
    
    Called after each source collection with the set of external_ids that
    were returned by the source API. Any active listing from that source
    NOT in the set is marked inactive (it was removed from the source).
    """
    if not current_external_ids:
        return 0
    
    placeholders = ",".join(["?"] * len(current_external_ids))
    result = db.execute(f"""
        UPDATE listings 
    ...
    """, [source] + list(current_external_ids))
    db.commit()
    
    deactivated = result.rowcount
    if deactivated > 0:
        print(f"    Deactivated {deactivated} stale listings from {source}")
    return deactivated


def reactivate_listing(db, external_id):
    """
    Reactivate a listing that reappeared on the source.
    Called during insert when a listing already exists but was inactive.
    """
    db.execute("""
        UPDATE listings 
        SET is_active = 1, deactivated_at = NULL, scraped_at = datetime('now')
...
def run():
    ...
    for name, collector in [
        ...
        try:
            ...
            # Deactivate listings no longer present on source
            current_ids = {l["external_id"] for l in normalized}
            deactivate_stale_listings(db, name, current_ids)
        except Exception as e:
