 ...
 *
 * Errors propagate to the caller. The watch-loop call site owns the
 * "log and continue" policy so this wrapper stays contract-consistent
...
export async function clusterNewDuplicates(db: Db): Promise<void> {
  ...
  if (result.clustered === 0) {
    console.log(`${t()} [dedup-inc] Nothing to cluster (${ms} ms)`);
    return;
  }
  console.log(
