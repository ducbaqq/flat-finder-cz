...
export class RealitymixScraper extends BaseScraper {
  ...
  private async *fetchCategoryPages(
    ...
    const firstPageUrl = `${this.baseUrl}/reality/${slug}?stranka=1`;
  ...
  override async enrichListings(
    ...
    const withGallery = listings.filter((l) => { try { return JSON.parse(l.image_urls).length > 3; } catch { return false; } }).length;
    this.log(`Enrichment: ${withGps}/${listings.length} GPS, ${withDesc}/${listings.length} description, ${withGallery}/${listings.length} gallery`);
  }
  ...
  private buildResult(
    ...
    // --- Additional params ---
    const formDiv2 = item.querySelector("[data-id]");
    const extraParams: Record<string, unknown> = {};
    const fkRk = formDiv2?.getAttribute("data-fk_rk");
    if (fkRk) extraParams.fk_rk = fkRk;
    const galleryLen = formDiv2?.getAttribute(
      "data-form-gallery-images-lenght",
    );
    if (galleryLen) extraParams.gallery_count = parseInt(galleryLen, 10);
    const formAddress = formDiv2?.getAttribute("data-form-address");
