...
export class IdnesScraper extends BaseScraper {
  ...
  private async fetchAjaxPage(category: Category, page: number): Promise<string | null> {
    const url = `${this.baseUrl}/s/${category.transactionSlug}/${category.propertySlug}/?page=${page}`;

    try {
      const data = await this.http.get<AjaxResponse>(url, {
        "X-Requested-With": "XMLHttpRequest",
        Referer: `${this.baseUrl}/s/${category.transactionSlug}/${category.propertySlug}/`,
      });
