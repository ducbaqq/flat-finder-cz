...
const BASE_URL = "https://www.eurobydleni.cz";
...
export class EurobydleniScraper extends BaseScraper {
  ...
  readonly sourceName = "eurobydleni";
  ...
  private async *fetchCategoryPages(
    ...
    const firstUrl = `${BASE_URL}/${urlPath}/`;
    ...
    const totalPages = Math.min(this.extractTotalPages(firstHtml), this.maxPages);
    ...
    for (let page = 2; page <= totalPages; page++) {
      const url = `${BASE_URL}/${urlPath}/page-${page}/`;
  ...
  private parseListingElement(
    ...
    const sourceUrl = detailHref ? `${BASE_URL}${detailHref}` : null;
  ...
  async enrichListings(listings: ScraperResult[]): Promise<ScraperResult[]> {
    ...
    this.log(`Enrichment complete: ${withGps}/${enriched.length} with GPS`);
    return enriched;
  }
  ...
  private async enrichOne(listing: ScraperResult): Promise<ScraperResult> {
    ...
    if (!numericId) return listing;

    let data: PropertyApiResponse;
    try {
      data = await this.http.get<PropertyApiResponse>(
        `${BASE_URL}/mlift/api/property.php?propertyId=${numericId}`,
      );
    } catch (err) {
    ...
    if (data.setUrl) listing.source_url = `${BASE_URL}${data.setUrl}`;
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const scraper = new EurobydleniScraper({ name: "eurobydleni", rps: getRps("eurobydleni"), maxPages: parseMaxPages() });
