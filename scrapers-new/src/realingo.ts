...
export class ReaLingoScraper extends BaseScraper {
  ...
  async enrichListings(listings: ScraperResult[]): Promise<ScraperResult[]> {
    ...
    const withExtUrl = enriched.filter((l) => {
      try {
        const params = l.additional_params ? JSON.parse(l.additional_params) : null;
        return params?.realingo_url != null;
      } catch {
        return false;
      }
    }).length;
    ...
    this.log(
      `Enrichment: ${withDesc}/${enriched.length} description, ${withExtUrl}/${enriched.length} external URL, ${withGallery}/${enriched.length} gallery`,
    );
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const scraper = new ReaLingoScraper({ name: "realingo", rps: getRps("realingo"), maxPages: parseMaxPages() });
